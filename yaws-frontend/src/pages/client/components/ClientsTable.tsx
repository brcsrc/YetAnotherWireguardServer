...
const ClientsTable = ({ network, showCreateButton }: ClientsTableProps): React.ReactNode => {
  ...
  const [preferences, setPreferences] = useState({
    pageSize: 10,
    visibleContent: ["name", "publicKeyValue", "cidr", "tag"],
  });
  ...
  const { items, collectionProps, propertyFilterProps } = useCollection(clients, {
    sorting: {},
    filtering: {
      empty: <div style={{ textAlign: "center", padding: "20px" }}>No clients found.</div>,
  ...
  return (
    <Table
      {...collectionProps}
      filter={<PropertyFilter {...propertyFilterProps} {...CLIENTS_TABLE_FILTER_PROPS} />}
      header={
        <Header
          counter={clients.length ? `(${clients.length})` : undefined}
          actions={
            showCreateButton ? (
              <Button
